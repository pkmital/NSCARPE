...
void ofxQTKitRecorder::addFrame(ofPixels& framePixels, float frameRate) {
    if (qtMovie != nil) {
        if (previousFrame.isAllocated()) {
            if (frameRate > 0) {
                outputFrame(previousFrame, frameRate);
            }
            else {
                ofLog(OF_LOG_ERROR, "ofxQTKitRecorder cannot draw 0 ms length frame");
            }
        }
        else
        {
            ofLog(OF_LOG_ERROR, "ofxQTKitRecorder does not have any previous frames allocated");
        }
        previousFrame = framePixels;
    }   else {
        ofLog(OF_LOG_ERROR, "QTMovie is NIL");
    }
}

void ofxQTKitRecorder::addFrame(ofPixels& framePixels, unsigned long msPerFrame) {  
    if (qtMovie != nil) {  
        if (previousFrame.isAllocated()) {  
            if (msPerFrame > 0) {  
                outputFrame(previousFrame, msPerFrame);  
            }
            else {
                ofLog(OF_LOG_ERROR, "ofxQTKitRecorder cannot draw 0 ms length frame");
            }
...
void ofxQTKitRecorder::loadAudioTrack(const char* filename) {
...
void ofxQTKitRecorder::addAudioTrack(const char* videoPath) {
...
void ofxQTKitRecorder::outputFrame(ofPixels& framePixels, float frameRate) {
    assert(frameRate > 0);
    QTTime duration = QTMakeTime(100, frameRate*100);
